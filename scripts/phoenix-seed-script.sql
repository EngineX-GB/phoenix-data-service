...
set d2.username = d1.username,
	...
	insert into tbl_client (username, nationality, location, rating, age, rate_15_min, rate_30_min, rate_1_hour, rate_1_50_hour, rate_2_hour, rate_2_50_hour, rate_3_hour, rate_3_50_hour, rate_4_hour, rate_overnight, telephone, url_page, refresh_time, user_id, image_available, region, gender, member_since, height, dress_size, hair_colour, eye_colour)
			(select username, nationality, location, rating, age, rate_15_min, rate_30_min, rate_1_hour, rate_1_50_hour, rate_2_hour, rate_2_50_hour, rate_3_hour, rate_3_50_hour, rate_4_hour, rate_overnight, telephone, url_page, refresh_time, user_id, image_available, region, gender, member_since, height, dress_size, hair_colour, eye_colour from tbl_client_temp where user_id not in (select user_id from tbl_client));
...
create procedure prc_fix_image_path()
begin
	declare i_oid integer;
	declare i_client_oid integer;
	declare i_image_path text;
	declare i_user_id integer;
	declare extension_position integer;
	declare image_value text;
declare cursor_query cursor for select i.oid, i.client_oid, i.image_path, c.user_id from tbl_client_image i inner join tbl_client c on c.oid = i.client_oid where i.image_path like '%http://ci.adultwork%';

	open cursor_query;
	get_flagged: LOOP
	fetch cursor_query into i_oid, i_client_oid, i_image_path, i_user_id;
	set @extension_position := INSTR(i_image_path, ".jpg");
	set @image_value := substring(i_image_path, @extension_position -1, @extension_position);
	update tbl_client_image i set i.image_path = concat("/", i_user_id, "/", i_user_id, "_", @image_value) where i.oid = i_oid;
	END LOOP get_flagged;
	close cursor_query;
end;
...
-- large :https://ci.adultwork.com/f/4371974_1.jpg
-- medium: https://ci.adultwork.com/i/4371974_1.jpg
-- small : https://ci.adultwork.com/t/4371974_1.jpg
